...
const BanHangPage = (props) => {
	const columns = [
		...
		},
		{
			...
			fixed: 'right',
			width: 70,
			render: (_, record) => <Button size='small' onClick={() => editRow(record.key)}>Sửa</Button>
		}
	];
  ...
  const handleChange = (key, field, value) => {
	...
	const renderCell = (text, record, index, column) => {
		...
		return ['price', 'discountAmount', 'totalPrice'].includes(column.dataIndex)
			? formatMoney(text)
			: text;
	};
