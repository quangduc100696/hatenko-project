...
import FlightReview from 'containers/Email/FlightReview';

const BookingFlightPreview = ({
  record,
  show = false
}) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const html = await render(
        <FlightReview owrnerInfo={owrnerInfo} management={management} record={record} />, { pretty: true }
      );
      setHtml(html);
    })();
...
export default BookingFlightPreview;